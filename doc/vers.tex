...
replicas it is only permissible to discard the contents of a file if
its modification history is a prefix of another.
...
cases where the same pathname has been used for a file on one replica
and a directory on another.  Second, it allowed us to simplify the
implementation, so that at its core pFS only really has one complex
function, \texttt{pfs\_set\_entry} (discussed in
...
pFS assigns each file and directory a
globally unique ID generated upon creation by hashing the device ID
and 

by
applying a 64-bits md5 cryptographic hash function on the
concatenation of the identifiant of the $sd$ where the ressource is
created and an incremented counter maintained on each $sd$.
