...
on one replica and a directory on another.  Second, it allowed us to
...
even when modified.  (Intuitively, this is because pFS versions
directory entries individually and can reconcile concurrent updates to
different files in the same directory, while concurrent updates to the
same file are always considered a conflict.)
...
name onto a set of \texttt{pfs\_ver} structures.  

, a directory is represented in pFS
